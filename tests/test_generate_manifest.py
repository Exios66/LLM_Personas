...
def test_parse_basename_unknown_falls_back():
    date_display, title = parse_basename("random_filename")
    assert date_display == "Unknown"
    assert title == "Random Filename"


def test_extract_case_number_prefers_case_no():
    content = "**Case No.**: 2026-SECU-042-001\n**Matter ID**: 2026-OLD-001\n"
    assert extract_case_number(content) == "2026-SECU-042-001"


def test_extract_case_number_legacy_matter_id():
    content = "**Matter ID**: 2026-LEGAL-001\n"
    assert extract_case_number(content) == "2026-LEGAL-001"


def test_extract_case_number_missing():
    assert extract_case_number("No header here.") is None
