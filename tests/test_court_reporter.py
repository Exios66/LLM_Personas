...
def test_suggest_canonical_name_from_header_date(tmp_path: Path) -> None:
    path = tmp_path / "legacy_notes.md"
    content = "**Date**: 2026-03-01\n"
    suggested = _suggest_canonical_name(path, content)
    assert suggested == "2026-03-01-legacy-notes.md"


def test_do_renames_renames_uncertified_non_canonical(
    tmp_path: Path,
) -> None:
    d = tmp_path / "t"
    d.mkdir()
...
def test_do_renames_skips_when_target_exists(tmp_path: Path) -> None:
    ...
    suggestion = _suggest_canonical_name(legacy, legacy.read_text(encoding="utf-8"))
    assert suggestion is not None
    (d / suggestion).write_text("already here\n", encoding="utf-8")
