...
2. **State backup (recommended):** Before major sessions, copy `state/current.md` to `state/backups/YYYY-MM-DD-current.md`. See [Error-Recovery](/guide/error-recovery.html).
...
When **not** to convene: F0 trivial, pure implementation, already decided by precedent, formatting-only, or R/Quarto-only (hand to OCTAVIUS). See [When-to-Convene](/guide/when-to-convene.html).

**Checklists:** `checklists/judge-morningstar.md` (presiding), `checklists/courtroom-scribe.md` (transcripts).

**See also:** [The-Court](/guide/the-court.html) · [State-and-Metrics](/guide/state-and-metrics.html)
