...
**Dissolution Protocol** — When *not* to convene (F0, pure implementation, already decided, etc.). See [When-to-Convene](/guide/when-to-convene.html).
...
**Handoff** — Formal transfer between agents (e.g. MORNINGSTAR → LIL_JEFF or OCTAVIUS). See [Inter-Agent-Protocol](/guide/inter-agent-protocol.html).
...
**Transcript** — Record of an F3+ deliberation. Stored in `courtroom/transcripts/`. View via [Portal](/guide/portal.html).
