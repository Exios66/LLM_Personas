...
| No `.html` for a given `.md` | Launch script exports on demand via Python. | Requires Python 3. If export fails, run export command manually (see [Runbook](/guide/runbook.html)). |
...
| State invalid (missing section) | Validation may fail; recovery procedures apply. | Backup state before major sessions; see [Error-Recovery](/guide/error-recovery.html). |
...
**See:** [Runbook](/guide/runbook.html) · [Error-Recovery](/guide/error-recovery.html)
